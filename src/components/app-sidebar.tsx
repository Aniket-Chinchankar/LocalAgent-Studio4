...
import { LayoutDashboard, MessageSquare, Database, Activity, Settings, Plus, LogOut, Sparkles, Trash2 } from "lucide-react";
...
import { listConversations, createConversation, deleteConversation } from "@/lib/conversations.functions";
              ...
              {convs.map((c) => (
                <SidebarMenuItem key={c.id}>
                  <SidebarMenuButton asChild isActive={pathname === `/chat/${c.id}`}>
                    <Link to="/chat/$conversationId" params={{ conversationId: c.id }} className="group">
                      <MessageSquare />
                      <span className="truncate">{c.title}</span>
                      <button
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); delMut.mutate(c.id); }}
